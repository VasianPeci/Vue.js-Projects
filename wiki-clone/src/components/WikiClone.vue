    ...
    <div :class="{'dark-theme': isDarkTheme}">
        <div class="container">
            ...
            <form id="search-form" @submit.prevent="submitSearch">
                <input id="search-input" type="text" v-model="searchQuery" placeholder="Enter search term">
                <button type="submit">Search</button>
            </form>
            <div class="search-result">
                <div class="spinner" v-if="isLoading">Loading...</div>
                <p v-if="searchResults.length">
                    <div class="result-item" v-for="result in searchResults" :key="result.pageid">
                        <h3 class="result-title">
                            <a :href="`https://en.wikipedia.org/?curid=${result.pageid}`" rel="noopener" target="_blank">{{ result.title }}</a>
                        </h3>
                        <a :href="`https://en.wikipedia.org/?curid=${result.pageid}`" class="result-link" rel="noopener" target="_blank">{{ `https://en.wikipedia.org/?curid=${result.pageid}` }}</a>
